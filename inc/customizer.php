 ...
 */
function achtermaerz_customize_register( $wp_customize ) {
	...
	$wp_customize->selective_refresh->add_partial( 'blogname', array(
		...
		'render_callback' => 'achtermaerz_customize_partial_blogname',
	) );
	...
	$wp_customize->add_setting( 'colorscheme', array(
		...
		'sanitize_callback' => 'achtermaerz_sanitize_colorscheme',
	) );
	...
	$wp_customize->add_setting( 'page_layout', array(
		'default'           => 'two-column',
		'sanitize_callback' => 'achtermaerz_sanitize_page_layout',
		'transport'         => 'postMessage',
	) );
	...
	$wp_customize->add_control( 'page_layout', array(
		...
		'active_callback' => 'achtermaerz_is_view_with_layout_option',
	) );
	 ...
	 */
	$num_sections = apply_filters( 'achtermaerz_front_page_sections', 4 );
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
			'active_callback' => 'achtermaerz_is_static_front_page',
		) );

		$wp_customize->selective_refresh->add_partial( 'panel_' . $i, array(
			'selector'            => '#panel' . $i,
			'render_callback'     => 'achtermaerz_front_page_section',
			'container_inclusive' => true,
		) );
	}
}
add_action( 'customize_register', 'achtermaerz_customize_register' );
 ...
 */
function achtermaerz_sanitize_page_layout( $input ) {
	$valid = array(
		'one-column' => __( 'One Column', 'twentyseventeen' ),
		'two-column' => __( 'Two Column', 'twentyseventeen' ),
	);
 ...
 */
function achtermaerz_sanitize_colorscheme( $input ) {
 ...
 * @return void
 */
function achtermaerz_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 */
function achtermaerz_is_static_front_page() {
 ...
 */
function achtermaerz_is_view_with_layout_option() {
 ...
 */
function achtermaerz_customize_preview_js() {
...
add_action( 'customize_preview_init', 'achtermaerz_customize_preview_js' );

/**
 * Load dynamic logic for the customizer controls area.
 */
function achtermaerz_panels_js() {
	wp_enqueue_script( 'twentyseventeen-customize-controls', get_theme_file_uri( '/assets/js/customize-controls.js' ), array(), '1.0', true );
}
add_action( 'customize_controls_enqueue_scripts', 'achtermaerz_panels_js' );
