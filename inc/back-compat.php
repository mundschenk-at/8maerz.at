 ...
 * @since Twenty Seventeen 1.0
 */
function achtermaerz_switch_theme() {
	switch_theme( WP_DEFAULT_THEME );
	unset( $_GET['activated'] );
	add_action( 'admin_notices', 'achtermaerz_upgrade_notice' );
}
add_action( 'after_switch_theme', 'achtermaerz_switch_theme' );
 ...
 */
function achtermaerz_upgrade_notice() {
 ...
 * @global string $wp_version WordPress version.
 */
function achtermaerz_customize() {
...
add_action( 'load-customize.php', 'achtermaerz_customize' );
...
add_action( 'template_redirect', 'achtermaerz_preview' );
